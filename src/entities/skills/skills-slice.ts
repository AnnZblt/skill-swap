...
// по подкатегории - передать аргументом номер категории, по которому производить выдачу
export const getCardsBySubcategory = (subcategoryId: number) =>
  createSelector([skillCards], (cards) =>
    cards.filter((c) => c.canTeach.subcategoryId === subcategoryId)
  );
...
// мои навыки - текущий пользователь = skillOwner.id
// передать аргументов id пользователя
export const getMyCards = (userId: string) =>
  createSelector([skillCards], (cards) =>
    cards.filter((c) => c.skillOwner.id === userId)
  );
